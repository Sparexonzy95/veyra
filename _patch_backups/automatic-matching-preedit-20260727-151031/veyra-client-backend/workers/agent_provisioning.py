...
from workers.automatic_qualification import (
    ...
    ensure_automatic_qualification,
)
...
def _runtime_ready(worker: WorkerAgent) -> bool:
    ...
    # The authenticated hosted connection and its fresh heartbeat are the
    # source of truth. WorkerAgent.engine_connected is a denormalized display
    # field and may be stale after a failed provisioning attempt or restart.
...
def provision_agent(
    ...
    *,
    ...
    if not worker.test_assignment_passed:
        try:
            ensure_automatic_qualification(worker)
        except AutomaticQualificationError as exc:
            _set_stage(
                worker,
                stage="QUALIFICATION_FAILED",
                status_value=WorkerAgent.Status.READY_FOR_QUALIFICATION,
