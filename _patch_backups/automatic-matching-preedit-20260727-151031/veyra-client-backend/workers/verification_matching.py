...
def rank_verifiers_for_assignment(
    ...
    *,
    ...
    for verifier in verifiers:
        ...
        skill_score = 2000 if not required else int(2000 * len(matched) / len(required))
        availability = 1000 if _active_reviews(verifier) == 0 else 300
        fairness = max(0, 1000 - min(recent_reviews, 10) * 100)
        score = skill_score + availability + fairness
        candidates.append(
            RankedVerifier(
        ...
        try:
            with transaction.atomic():
                ...
                if current and current.status not in {
                    ...
                    return current
                verifier = WorkerAgent.objects.select_for_update().select_related(
                    "hosted_connection"
