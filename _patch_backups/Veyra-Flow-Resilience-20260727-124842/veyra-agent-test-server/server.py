...
ENV_FILE = Path(os.getenv("VEYRA_RUNTIME_ENV_FILE", str(ROOT / ".env"))).resolve()
load_dotenv(ENV_FILE)
STATE_DIR = Path(
    os.getenv("VEYRA_RUNTIME_STATE_DIR", str(ROOT / ".veyra-runtime"))
).resolve()
...
RUNTIME_VERSION = f"veyra-owner-runtime-{RUNTIME_ROLE.lower()}/1.1.0"
PROTOCOL_VERSION = 1
...
def _job_workspace(assignment_id: str) -> Path:
    safe_id = re.sub(r"[^A-Za-z0-9_.-]", "", assignment_id)[:80]
    return STATE_DIR / "jobs" / safe_id
...
def _path_under(path: str, prefix: str) -> bool:
    clean_path = path.casefold().strip("/")
    clean_prefix = prefix.casefold().strip("/")
...
def _enforce_job_path_policy(task: dict[str, Any], paths: list[str]) -> None:
    ...
    for path in paths:
        ...
        if basename == ".env" or basename.startswith(".env."):
            raise RuntimeError(f"The model attempted to change protected environment path: {selected}")
        if any(_path_under(selected, prefix) for prefix in forbidden):
            raise RuntimeError(f"The model attempted to change protected path: {selected}")
        if allowed and not any(_path_under(selected, prefix) for prefix in allowed):
            raise RuntimeError(f"The model attempted to change a path outside the funded policy: {selected}")
        ...
        if not bool(policy.get("allow_new_dependencies")) and Path(lower).name in {
            ...
            raise RuntimeError(f"Dependency changes are disabled by policy: {selected}")
        if not bool(policy.get("allow_database_migrations")) and (
            "/migrations/" in f"/{lower}/" or lower.startswith("migrations/")
        ):
            raise RuntimeError(f"Database migrations are disabled by policy: {selected}")
...
def _extract_model_files(content: str) -> tuple[list[dict[str, str]], str]:
...
def _run_job_model(
    ...
    *,
    ...
    repair = (
        "\n\nThe previous test attempt failed. Fix the code using this sanitized output:\n"
    ...
    prompt = (
        ...
        '{"summary":"brief summary","files":[{"path":"relative/path","content":"complete file content"}]}. '
        "Return only files that must change, with complete replacement content. "
        "Do not touch protected paths. Do not include markdown fences.\n\n"
    ...
    response = httpx.post(
        ...
        json={
            "model": AI_MODEL,
            "messages": [
                {"role": "system", "content": "Return only valid JSON with complete changed files."},
                {"role": "user", "content": prompt},
            ],
            "temperature": 0.1,
        },
    ...
    content = message.get("content") if isinstance(message, dict) else ""
    return _extract_model_files(str(content or ""))
...
def _changed_files(workspace: Path) -> list[str]:
    ...
    for line in output.splitlines():
        ...
        if value:
            result.append(_safe_path(value))
...
def run_job_task(task: dict[str, Any]) -> None:
    ...
    with STATE_LOCK:
        ...
        save_state(state)
    workspace = _job_workspace(assignment_id)
    token = ""
    try:
        ...
        files, summary = _run_job_model(task, workspace)
        ...
        while code != 0 and repairs < max_repairs:
            repairs += 1
            files, summary = _run_job_model(task, workspace, previous_test_output=output)
            _apply_model_files(task, workspace, files)
            code, output, command = _run_validation_commands(task, workspace)
        if code != 0:
        ...
        if os.getenv("VEYRA_KEEP_FAILED_WORKSPACES", "false").casefold() not in {"1", "true", "yes"}:
            shutil.rmtree(workspace, ignore_errors=True)
        with JOB_LOCK:
            JOB_RUNNING.discard(assignment_id)
...
def ensure_job_thread(task: dict[str, Any]) -> None:
    ...
    if not assignment_id:
        return
    with JOB_LOCK:
        if assignment_id in JOB_RUNNING:
            return
        state = load_state()
        if (
...
def _verification_workspace(verification_id: str) -> Path:
    ...
    return STATE_DIR / "verification" / safe_id
