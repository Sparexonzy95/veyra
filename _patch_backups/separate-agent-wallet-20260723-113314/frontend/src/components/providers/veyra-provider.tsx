...
export function VeyraProvider({ children }: { children: React.ReactNode }) {
  ...
  const prepareWallet = useCallback(async (session: CircleSession, redirectPath = "/dashboard") => {
    setWalletSetupOpen(true);
    setStatus("Setting up your secure Veyra wallet…");
    const body = {
    ...
    for (let attempt = 0; attempt < 10; attempt += 1) {
      try {
        ...
        if (nextMe.authenticated) {
          toast.success("Your Veyra wallet is ready.");
  ...
  const chooseClientRole = useCallback(async () => {
    ...
    try {
      ...
      await prepareWallet(session, "/dashboard");
    } catch (roleError) {
      setWalletSetupOpen(false);
  ...
  const chooseAgentOwnerRole = useCallback(async () => {
    ...
    try {
      ...
      await prepareWallet(session, "/dashboard/agents");
    } catch (roleError) {
      setWalletSetupOpen(false);
