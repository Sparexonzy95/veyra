...
class WorkerAgent(TimeStampedModel):
    ...
    auto_claim_enabled = models.BooleanField(default=False)
    ...
    def clean(self):
        ...
        if self.auto_claim_enabled and self.status != self.Status.ACTIVE:
            errors["auto_claim_enabled"] = "Auto-claim is available only after the agent is active."
        ...
        if self.status == self.Status.ACTIVE:
            ...
            if missing:
                errors["status"] = (
                    "Worker cannot be ACTIVE until onboarding is complete: "
                    + ", ".join(missing)
                )

        if self.discovery_enabled and self.status != self.Status.ACTIVE:
            errors["discovery_enabled"] = "Only an ACTIVE worker can discover jobs."
...
class RunnerDevice(TimeStampedModel):
    """A cryptographic identity for one owner-hosted Veyra Runner.

    The device address is derived from a runner-only keypair. It is never used
    as an Arc wallet and never holds funds. The corresponding private key stays
    on the owner's machine or server and is never sent to Veyra.
    """
...
class RunnerAgentBinding(TimeStampedModel):
    """Links one agent to one runner while allowing a runner to host many agents."""
...
class WorkerJobQueueItem(TimeStampedModel):
    ...
    def clean(self):
        ...
        if self.status in {self.Status.SUBMISSION_PENDING, self.Status.SUBMITTED}:
