...
class JobDraftSerializer(serializers.ModelSerializer):
    class Meta:
        model = JobDraft
        fields = [
            'id', 'status', 'github_issue_url', 'repository_owner', 'repository_name', 'target_branch',
            'issue_number', 'issue_title', 'issue_body', 'budget_usdc', 'deadline',
            'acceptance_criteria', 'advanced_options', 'created_at', 'updated_at',
        ]
        read_only_fields = ['id', 'status', 'repository_owner', 'repository_name', 'target_branch', 'issue_number', 'issue_title', 'issue_body', 'created_at', 'updated_at']
    ...
    def validate_deadline(self, value):
        ...
        if seconds < 600:
            raise serializers.ValidationError('Deadline must be at least 10 minutes from now.')
