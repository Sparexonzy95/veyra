...
def preflight_controlled_test_runtime(
    ...
    *,
    ...
    if login.casefold() != worker.github_username.casefold():
        raise WorkerTestAssignmentError(
            "github_auth",
            "The live GitHub account does not match the worker record.",
        )
...
def prepare_controlled_test_assignment(
    ...
    *,
    ...
    required = {
        ...
        "github_connected": worker.github_connected,
    ...
    github = github_client or GitHubWorkerClient()
    check_github_bot(expected_username=worker.github_username)
    ...
    assignment = WorkerTestAssignment.objects.create(
        ...
        fork_owner=worker.github_username,
...
def execute_controlled_test_assignment(
    ...
    *,
    ...
    if str(authenticated.get("login") or "").casefold() != worker.github_username.casefold():
        raise WorkerTestAssignmentError(
            "github_auth",
            "The live GitHub account no longer matches the worker record.",
        )
    ...
    try:
        ...
        noreply_email = (
            f"{authenticated['id']}+{worker.github_username}@users.noreply.github.com"
        )
        ...
        assignment.fork_owner = worker.github_username
        ...
        expected_clone_url = (
            f"https://github.com/{worker.github_username}/{assignment.source_repository}.git"
        )
        ...
        with _git_auth_environment(username=worker.github_username, token=github.token) as git_env:
            push = command_runner(
                [
                    git,
                    "push",
