...
def check_worker_readiness(
    ...
    *,
    ...
    github_record_ready = bool(worker.github_connected and worker.github_username)
    checks.append(
        _check(
            "github_record",
            github_record_ready,
            f"Stored GitHub account is {worker.github_username}.",
            "GitHub account is not connected in the worker record.",
        )
    )

    if github_record_ready:
        try:
            github_result = github_checker(expected_username=worker.github_username)
            checks.append(
                _check(
                    "github_live",
                    github_result.connected,
                    f"GitHub authenticated as {github_result.login}.",
                    "GitHub token verification did not succeed.",
                )
            )
        except Exception as exc:
            checks.append(
                ReadinessCheck(
                    name="github_live",
                    passed=False,
                    detail=f"GitHub verification failed: {_safe_error(exc)}",
                )
            )
    else:
        checks.append(
            ReadinessCheck(
                name="github_live",
                passed=False,
                detail="GitHub live verification was skipped because the account is not connected.",
            )
        )
