...
class GitHubFreshnessGuard:
    ...
    def check(self, worker: WorkerAgent, job: VeyraJob) -> GitHubFreshnessResult:
        ...
        job = VeyraJob.objects.select_related("draft").get(pk=job.pk)
        ...
        if not worker.github_connected or not worker.github_username:
            raise GitHubFreshnessError(
                "The worker GitHub account is not connected."
            )
        ...
        with httpx.Client(
            ...
            for pull in self._list_pulls(
                ...
                if self._pull_head_owner(pull).casefold() != worker.github_username.casefold():
                    continue
                ...
                if merged or pull_state == "OPEN":
                    ...
                    detail = (
                        "The worker already merged a pull request for this issue."
                        if merged
                        else "The worker already has an open pull request for this issue."
                    )
                    return GitHubFreshnessResult(
                        passed=False,
                        code=code,
                ...
                for branch in self._list_branches(
                    ...
                    if branch.casefold().startswith("veyra/") and branch_pattern.search(branch):
                        return GitHubFreshnessResult(
                            ...
                            detail="The worker already has a Veyra branch for this issue.",
                            issue_state=issue_state,
                            issue_url=authoritative_issue_url,
        ...
        return GitHubFreshnessResult(
            passed=True,
            code="GITHUB_FRESH",
            detail="The GitHub issue is open with no existing worker pull request or branch.",
            issue_state="OPEN",
            issue_url=issue_url,
            checked_at=checked_at,
        )
