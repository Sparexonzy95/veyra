...
def _submit_settlement_transaction(
    *,
    ...
    if approved:
        function = arc.contract.functions.verifyAndPay(
        ...
        function = arc.contract.functions.rejectAndRefund(
            int(assignment.job.onchain_job_id),
            bytes.fromhex(assignment.queue_item.submission_deliverable_hash.removeprefix("0x")),
            bytes.fromhex(report_hash.removeprefix("0x")),
    ...
    try:
        ...
        try:
            ...
            tx_data["gas"] = 500_000
        try:
            tx_data["gasPrice"] = int(arc.w3.eth.gas_price)
        except Exception:
            pass
        unsigned = function.build_transaction(tx_data)
        signed = signer.sign_transaction(unsigned)
        raw = getattr(signed, "raw_transaction", None) or getattr(signed, "rawTransaction", None)
        if raw is None:
            raise ExecutionVerificationError("The verifier signer returned no raw transaction.")
...
def _sync_reputation(assignment: WorkerJobAssignment, arc: ArcClient) -> WorkerReputationSnapshot:
    ...
    try:
        karma = int(arc.contract.functions.karmaScore(address).call())
        completed = int(arc.contract.functions.completedJobs(address).call())
        failed = int(arc.contract.functions.failedJobs(address).call())
        abandoned = int(arc.contract.functions.abandonedJobs(address).call())
        earned = int(arc.contract.functions.totalEarned(address).call())
    except Exception as exc:
        raise ExecutionVerificationPending(
    ...
    *,
    ...
    if onchain.get("status") in {"COMPLETED", "REJECTED"}:
        ...
        with transaction.atomic():
            ...
            item.save(update_fields=["status", "onchain_status", "updated_at"])
            job = locked.job
    ...
    if assignment.verification_report_hash:
        ...
        try:
            value = arc.contract.functions.computeEvidenceHash(
                int(assignment.job.onchain_job_id),
                bytes.fromhex(assignment.queue_item.submission_deliverable_hash.removeprefix("0x")),
                bytes.fromhex(report_hash.removeprefix("0x")),
                bool(approved),
    ...
    if not tx_hash:
        with transaction.atomic():
            ...
            if not locked.settlement_transaction_hash:
                tx_hash = _submit_settlement_transaction(
    ...
    try:
        receipt = arc.w3.eth.wait_for_transaction_receipt(tx_hash, timeout=timeout, poll_latency=poll)
    except Exception as exc:
        raise ExecutionVerificationPending(
