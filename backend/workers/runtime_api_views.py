...
def _delivery_error(stage: str, exc: Exception) -> dict[str, str]:
    message = (str(exc or "").strip() or exc.__class__.__name__)[:500]
...
def _authenticated_connection(request):
    ...
    connection = (
        ...
        .filter(
            worker_id=agent_id,
            status__in=[
                HostedAgentConnection.Status.CONNECTING,
...
class AgentRuntimeHeartbeatView(APIView):
    ...
    def post(self, request):
        ...
        qualification_delivery_failed = False
        try:
            ...
            qualification_delivery_failed = True
            delivery_errors.append(_delivery_error("qualification_delivery", exc))
        except Exception as exc:
            qualification_task = None
            qualification_delivery_failed = True
            logger.exception("Hosted runtime qualification delivery failed")
            delivery_errors.append(
                {
                    "stage": "qualification_delivery",
                    "message": "Veyra could not prepare the qualification task.",
                }
            )

        job_task = None
        verification_task = None
        if qualification_task is None and not qualification_delivery_failed:
            try:
                job_task = execution_task_for_connection(connection)
            except ExecutionTransportError as exc:
                delivery_errors.append(_delivery_error("job_delivery", exc))
            except Exception:
                logger.exception("Hosted runtime paid-job delivery failed")
                delivery_errors.append(
                    {
                        "stage": "job_delivery",
                        "message": "Veyra could not prepare the paid job for this runtime.",
                    }
                )
            if job_task is None:
                try:
                    ...
                    delivery_errors.append(_delivery_error("verification_delivery", exc))
                except Exception:
                    logger.exception("Hosted runtime verification delivery failed")
                    delivery_errors.append(
                        {
                            "stage": "verification_delivery",
                            "message": "Veyra could not prepare the verification task.",
                        }
                    )
        return Response(
            {
