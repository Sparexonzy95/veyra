...
from workers.runtime_pairing import create_pairing_code, revoke_agent_runtime


@override_settings(
    ROOT_URLCONF="workers.legacy_runner_test_urls",
...
class RuntimePairingTests(TestCase):
    ...
    def create_code(self, worker=None):
        ...
        code, _record = create_pairing_code(
            worker=worker,
            owner_user=self.owner,
        )
        return code
    ...
    def test_signature_nonce_cannot_be_replayed(self):
        ...
        self.assertEqual(RunnerRequestNonce.objects.count(), 1)
    ...
    def test_owner_can_revoke_only_the_agent_binding(self):
        ...
        self.client.force_authenticate(self.owner)
        response = self.client.get(f"/api/v1/agents/{self.worker.id}/")
        self.assertEqual(response.status_code, 200, response.data)
        self.assertEqual(response.data["runtime"]["status"], "REVOKED")
        self.assertFalse(response.data["onboarding"]["checks"]["runtime"])
