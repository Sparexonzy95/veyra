...
from workers.models import WorkerAgent, WorkerJobQueueItem, WorkerQualificationRun
...
def _qualification_is_current(worker: WorkerAgent) -> bool:
    if not worker.test_assignment_passed:
        return False
    if worker.owner_type == WorkerAgent.OwnerType.VEYRA:
        return True
    maximum_age = timedelta(
        days=max(
            1,
            int(getattr(settings, "VEYRA_QUALIFICATION_MAX_AGE_DAYS", 30)),
        )
    )
    latest = (
        WorkerQualificationRun.objects.filter(
            worker=worker,
            status=WorkerQualificationRun.Status.PASSED,
    ...
    return bool(latest and latest >= timezone.now() - maximum_age)
...
def evaluate_job(
    ...
    *,
    ...
    if not _qualification_is_current(worker):
        return _fail(
            "QUALIFICATION_EXPIRED",
            "The external worker qualification is missing or no longer current.",
        )
    ...
    repository_access = job.draft.github_repository_access
    committed_repository_id = int(repository.get("githubRepositoryId") or 0)
    if repository_access is not None and committed_repository_id not in {
        0,
        int(repository_access.github_repository_id),
    }:
        return _fail(
            "REPOSITORY_ACCESS_MISMATCH",
            "The committed GitHub repository ID does not match the approved installation.",
            required=required,
            matched=matched,
            onchain=onchain,
        )

    committed_visibility = str(
        repository.get("repositoryVisibility") or "PUBLIC"
    ).upper()
    actual_private = bool(repository_access.private) if repository_access else (
        committed_visibility == "PRIVATE"
    )
    if repository_access is not None:
        actual_visibility = "PRIVATE" if repository_access.private else "PUBLIC"
        if committed_visibility != actual_visibility:
            return _fail(
                "REPOSITORY_VISIBILITY_MISMATCH",
                "The committed repository visibility does not match GitHub access.",
                required=required,
                matched=matched,
                onchain=onchain,
            )
    if worker.public_repositories_only and actual_private:
        return _fail(
            "PRIVATE_REPOSITORY_NOT_ALLOWED",
            "This agent is configured for public repositories only.",
            required=required,
            matched=matched,
            onchain=onchain,
        )
    ...
    allowed_organizations = {
        ...
        for value in (worker.allowed_organizations or [])
        if str(value).strip()
    }
    if allowed_organizations and job.draft.repository_owner.casefold() not in allowed_organizations:
        return _fail(
            "REPOSITORY_OWNER_NOT_ALLOWED",
            "The repository owner is outside this agent's allowed organisations.",
            required=required,
            matched=matched,
            onchain=onchain,
        )
    ...
    if require_match and required and len(matched) != len(required):
        missing = [
            skill
            for skill in required
            if _normalise_skill(skill) not in {
                _normalise_skill(value) for value in matched
            }
        ]
        return _fail(
            "SKILL_NOT_VERIFIED",
            "The worker has not qualified for every required repository skill: "
            + ", ".join(missing),
            required=required,
            matched=matched,
            onchain=onchain,
...
def _capacity_used(worker: WorkerAgent, *, exclude_item_id=None) -> int:
    return active_assignment_count(
        worker,
        exclude_queue_item_id=exclude_item_id,
    )
...
def discover_job(
    ...
    *,
    ...
    if result.passed:
        ...
        delay = min(300, 5 * (2 ** min(item.matching_retry_count - 1, 6)))
    ...
    if result.code not in {
        ...
        item.matching_retry_count = 0
        item.matching_next_retry_at = None
    item.save()
