...
class HostedAgentConnectionTests(TestCase):
    ...
    @override_settings(DEBUG=False, VEYRA_ALLOW_LOCAL_AGENT_RUNTIME=False)
    @patch("workers.hosted_agent_connection._host_addresses")
    def test_rechecks_dns_before_runtime_claim(self, host_addresses):
        host_addresses.side_effect = [
            {ipaddress.ip_address("8.8.8.8")},
            {ipaddress.ip_address("8.8.8.8")},
            {ipaddress.ip_address("127.0.0.1")},
        ]
        public_link = (
            "veyra-connect://runtime.example/connect/"
            "abcdefghijklmnopqrstuvwxyz123456?protocol=1"
        )
    ...
    def test_credential_hash_is_durable_before_remote_claim_ack(self):
        private_key = Ed25519PrivateKey.generate()
        public_key = b64url(
            private_key.public_key().public_bytes(
                encoding=serialization.Encoding.Raw,
        ...
        def handler(request: httpx.Request):
            ...
            if request.url.path == "/veyra/connect/challenge":
                challenge = payload["challenge"]
                runtime_id = "runtime-claim-interrupted"
                signature = private_key.sign(
                    f"veyra-connect-v1:{challenge}:{runtime_id}".encode("utf-8")
                )
                return httpx.Response(
            ...
            raise httpx.ConnectError("claim response was lost", request=request)
        ...
        connection = HostedAgentConnection.objects.get(worker=self.worker)
        self.assertEqual(connection.status, HostedAgentConnection.Status.CONNECTING)
        self.assertEqual(len(connection.credential_hash), 64)
        self.assertEqual(
            connection.metadata["claim_state"],
            "PENDING_REMOTE_ACK",
        )
