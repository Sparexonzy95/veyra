...
def connect_hosted_agent(
    *,
    ...
    runtime_hostname = urlparse(parsed.runtime_base_url).hostname or ""
    ...
    try:
        # Re-resolve immediately before every outbound connection. This closes
        # the broad validation/request gap used by DNS-rebinding SSRF attacks.
        _validate_runtime_host(runtime_hostname)
        ...
        if expected_role:
            advertised_role = str(
                (verified.get("capabilities") or {}).get("role") or ""
            ).strip().upper()
            required_role = str(expected_role).strip().upper()
            if advertised_role != required_role:
                raise HostedAgentConnectionError(
                    f"This runtime advertises role {advertised_role or 'UNKNOWN'}, "
                    f"but Veyra expected {required_role}."
                )
        ...
        with transaction.atomic():
            ...
            if (
                ...
                same_signing_key = (
                    existing_connection.public_key_fingerprint
                    == verified["public_key_fingerprint"]
                )
                if not same_signing_key or runtime_is_online(existing_connection):
                    raise HostedAgentConnectionError(
        ...
        # Persist the credential hash before the one-time remote claim can be
        # consumed. If the final local update fails, the runtime can still
        # authenticate its first heartbeat and complete the connection.
        with transaction.atomic():
            pending_connection, _ = (
                HostedAgentConnection.objects.select_for_update().get_or_create(
                    worker=worker,
                    defaults={
                        "runtime_id": verified["runtime_id"],
            ...
            pending_connection.runtime_id = verified["runtime_id"]
            pending_connection.runtime_url = parsed.runtime_base_url
            pending_connection.public_key = verified["public_key"]
            pending_connection.public_key_fingerprint = verified[
                "public_key_fingerprint"
            ]
            pending_connection.protocol_version = verified["protocol_version"]
            pending_connection.runtime_version = verified["runtime_version"]
            pending_connection.provider = verified["provider"]
            pending_connection.model_name = verified["model"]
            pending_connection.provider_ready = verified["provider_ready"]
            pending_connection.capabilities = verified["capabilities"]
            pending_connection.credential_hash = _credential_hash(runtime_credential)
            pending_connection.status = HostedAgentConnection.Status.CONNECTING
            pending_connection.last_error = ""
            pending_connection.metadata = {
                "provider_message": verified["provider_message"],
                "connection_method": "COPY_LINK_V1",
                "claim_state": "PENDING_REMOTE_ACK",
            }
        ...
        with transaction.atomic():
            ...
            if (
                ...
                if not same_signing_key or runtime_is_online(connection):
                    raise HostedAgentConnectionError(
                        "This agent is already connected to a different hosted runtime."
                    )
