...
def _sanitised_test_environment(workspace: Path) -> dict[str, str]:
    """Build a minimal child environment for repository-controlled tests."""
...
def _run_pytest(workspace: Path, venv_python: Path) -> CommandResult:
    return _run_command(
        ...
        cwd=workspace,
        timeout=900,
        env=_sanitised_test_environment(workspace),
    )
