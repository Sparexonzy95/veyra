...
export default function GitHubCallbackPage() {
  ...
  const [returnPath, setReturnPath] = useState("/client/jobs");
  ...
  useEffect(() => {
    ...
    if (!stateValue && setupAction === "update") {
      setReturnPath("/client/jobs");
      setState("done");
      setMessage("GitHub repository access was updated. Veyra is synchronizing the latest repository list.");
      window.setTimeout(() => router.replace("/client/jobs"), 1200);
      return;
    }

    if (!stateValue) {
      setState("error");
      setMessage("Start the GitHub connection from Veyra so the installation can be linked securely to your account.");
      return;
    }
    ...
    void postJson<{
      ...
      .then((result) => {
        setReturnPath(result.return_path || "/client/jobs");
        setMessage(
        ...
        setState("done");
        window.setTimeout(() => router.replace(result.return_path || "/client/jobs"), 1200);
      })
      .catch((error) => {
        setState("error");
