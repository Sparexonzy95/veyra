...
class WorkspaceRetryTests(unittest.TestCase):
    ...
    def test_repository_process_does_not_inherit_runtime_secrets(self):
        os.environ["AI_API_KEY"] = "must-not-reach-child"
        os.environ["VEYRA_RUNTIME_CREDENTIAL"] = "must-not-reach-child"
        workspace = Path(self.temp_dir.name) / "process-env"
        workspace.mkdir()

        completed = self.server._run_process(
            [
                sys.executable,
                "-c",
                (
                    ...
                    "print(os.getenv('VEYRA_RUNTIME_CREDENTIAL', ''))"
                ),
            ],
            cwd=workspace,
            timeout=10,
