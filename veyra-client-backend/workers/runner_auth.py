...
from workers.models import RunnerDevice, RunnerRequestNonce
    ...
    def authenticate(self, request):
        ...
        try:
            with transaction.atomic():
                RunnerRequestNonce.objects.filter(
                    runner=runner,
                    expires_at__lte=now,
                ).delete()
                RunnerRequestNonce.objects.create(
                    runner=runner,
                    nonce_hash=nonce_hash,
                    expires_at=now + timedelta(seconds=max_skew * 2),
                )
