...
@override_settings(VEYRA_QUALIFICATION_MAX_AGE_DAYS=30)
class QualificationFreshnessTests(TestCase):
    def setUp(self):
        owner = User.objects.create_user(handle="qualification-owner")
        self.worker = WorkerAgent.objects.create(
            slug="qualified-external-worker",
            name="Qualified External Worker",
    ...
    def test_external_qualification_must_exist_and_be_current(self):
        self.assertFalse(_qualification_is_current(self.worker))
        run = WorkerQualificationRun.objects.create(
            worker=self.worker,
            status=WorkerQualificationRun.Status.PASSED,
            completed_at=timezone.now(),
        )
        self.assertTrue(_qualification_is_current(self.worker))

        WorkerQualificationRun.objects.filter(pk=run.pk).update(
...
class WorkerDiscoveryTests(TestCase):
    ...
    def test_skill_mismatch_is_ineligible(self):
        ...
        self.assertEqual(result.eligibility_code, "SKILL_NOT_VERIFIED")
    ...
    def test_active_assignment_defers_second_eligible_job(self):
        ...
        first_item = WorkerJobQueueItem.objects.get(pk=first_result.queue_item_id)
    ...
    def test_public_only_worker_rejects_private_client_repository(self):
        ...
        draft.github_repository_access = access
        draft.save(update_fields=["github_repository_access", "updated_at"])
        snapshot = draft.funding_snapshot
        commitment = dict(snapshot.repository_commitment)
        commitment["githubRepositoryId"] = 8003
        commitment["repositoryVisibility"] = "PRIVATE"
        snapshot.repository_commitment = commitment
        snapshot.save(update_fields=["repository_commitment"])
        self.worker.discovery_enabled = True
        self.worker.public_repositories_only = True
        self.worker.save(
            update_fields=[
                "discovery_enabled",
        ...
        result = discover_job(
            self.worker, self.job, arc_client=self.arc, github_guard=self.github
        )

        self.assertEqual(result.status, WorkerJobQueueItem.Status.INELIGIBLE)
        self.assertEqual(result.eligibility_code, "PRIVATE_REPOSITORY_NOT_ALLOWED")
