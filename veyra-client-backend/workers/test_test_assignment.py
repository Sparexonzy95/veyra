...
class WorkerTestAssignmentHelperTests(TestCase):
    ...
    def test_repository_test_environment_excludes_backend_secrets(self):
        with patch.dict(
            os.environ,
            {
                "CIRCLE_API_KEY": "circle-secret",
                "VEYRA_VERIFIER_PRIVATE_KEY": "verifier-secret",
                "GITHUB_APP_PRIVATE_KEY": "github-secret",
            },
        ):
            environment = _sanitised_test_environment(Path("workspace"))

        self.assertNotIn("CIRCLE_API_KEY", environment)
        self.assertNotIn("VEYRA_VERIFIER_PRIVATE_KEY", environment)
