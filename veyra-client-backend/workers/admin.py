...
@admin.register(WorkerJobAssignment)
class WorkerJobAssignmentAdmin(admin.ModelAdmin):
    list_display = (
        "job", "worker", "status", "matching_score", "candidate_count",
        "verification_status", "settlement_transaction_hash", "updated_at",
    )
    list_filter = ("status", "verification_status")
    search_fields = (
        "job__onchain_job_id", "worker__name", "worker__slug",
        "queue_item__execution_pull_request_url",
    )
    readonly_fields = tuple(field.name for field in WorkerJobAssignment._meta.fields)


@admin.register(WorkerReputationSnapshot)
class WorkerReputationSnapshotAdmin(admin.ModelAdmin):
    list_display = (
        "worker", "karma_score", "completed_jobs", "failed_jobs",
        "abandoned_jobs", "total_earned_atomic", "synced_at",
    )
    search_fields = ("worker__name", "worker__slug", "worker__worker_wallet_address")
    readonly_fields = tuple(field.name for field in WorkerReputationSnapshot._meta.fields)



@admin.register(WorkerVerificationAssignment)
class WorkerVerificationAssignmentAdmin(admin.ModelAdmin):
    list_display = (
        "worker_assignment",
        "verifier",
        "status",
    ...
    list_filter = ("status", "verdict", "verifier")
    search_fields = (
        "worker_assignment__job__onchain_job_id",
        "worker_assignment__worker__name",
        "verifier__name",
        "report_hash",
    )
    readonly_fields = tuple(
        field.name for field in WorkerVerificationAssignment._meta.fields
    )
